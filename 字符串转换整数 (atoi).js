 ...
 * 说明：
 * 
 * 假设我们的环境只能存储 32 位大小的有符号整数，那么其数值范围为 [−231,  231 − 1]。如果
 * 数值超过这个范围，请返回  INT_MAX (231 − 1) 或 INT_MIN (−231) 。
 * 
 * 示例 1:
 * 
 *  输入: "42"
...
var myAtoi = function (str) {
     ...
     * 正则匹配以及exec函数应用
     */
    let { 1: sign, 2: value } = /^\s*([\-\+])?([0-9]+)/g.exec(str) || {};
    sign = sign === '-' ? -1 : 1;
    if(value){
        value = sign*value;
        const max=Math.pow(2,31);
        return Math.max(-(max), Math.min(max-1, value))
    }else{
        return 0;
    }
};